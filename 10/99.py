...
def hello():
    name = "Hoge"
    #return name
    return render_template('hello.html', title='機械翻訳', name=name) #変更

@app.route("/calc", methods=["post"])
def calc():

    model = MyTransformer(device=device, max_len=max_len, source_vocab_length=V_ja, target_vocab_length=V_en).to(device)
    model.load_state_dict(torch.load('best_model_94.pt'))


    src = request.form["src"]
    greeedy_decode_sentence(model ,src)
 
    return greeedy_decode_sentence(model ,src)    
