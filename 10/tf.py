...
V_ja = get_vocab_ja()
V_en = get_vocab_en()
model = MyTransformer(device=device, max_len=42, source_vocab_length=V_ja, target_vocab_length=V_en).to(device)


optim = optim.Adam(model.parameters(), lr=0.01, betas=(0.9, 0.98), eps=1e-9)
...
def evaluation(valid_loader, model):
    ...
    with torch.no_grad():
        ...
        ex_ids = F.softmax(preds.transpose(0,1)[-1], dim=1).argmax(1).tolist()
        ex_sentence = tokenizer.tokenizer.DecodePieces(tokenizer.convert_ids_to_tokens(ex_ids))
        
    return total_loss / len_loader, ex_sentence
...
def train(train_loader, valid_loader, model, optim, epoch):
    ...
    for i, batch in tqdm(enumerate(train_loader, 1)):
        ...
        """
        if i % 10 == 0:
            valid_loss, ex_sentence = evaluation(valid_loader, model)
            print_list = [epoch[0], i, valid_loss]
            print(f"Epoch [{epoch[0]}/{epoch[1]}] Batch[{i}/{len_loader}] complete. valid_loss = {valid_loss}")
            # sentence
            print(f"Ex:\t{ex_sentence}")
            print(f"Post:\t{post}")
            print(f"Question:\t{greeedy_decode_sentence(model, sentence, padding_mask, device)}\n")
            # CSV書き出し
            with open(log_output_path + "valid_log.csv", mode="a") as out_f:
                csv.writer(out_f).writerow(print_list)
            # bestモデル保存
            if valid_loss < best_valid_loss:
                best_valid_loss, best_epoch, best_batch = valid_loss, epoch[0], i
                torch.save(model.state_dict(), f"{model_output_path}best_model.pt")
        ...
        """
        if i % 1000 == 0:
            torch.save(model.state_dict(), f"model_epoch{epoch[0]}_batch{i}.pt")
            print("models saved\n")
...
for epoch in range(1, EPOCH_NUM):
    success = train(train_loader, valid_loader, model, optim, [epoch, EPOCH_NUM])      
